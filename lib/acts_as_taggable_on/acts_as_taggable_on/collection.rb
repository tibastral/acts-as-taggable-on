    ...
    module ClassMethods
      ...
      def all_tag_counts(options = {})
        ...
        if ActiveRecord::VERSION::MAJOR >= 3
          ...
          tagging_scope = tagging_scope.group("#{ActsAsTaggableOn::Tagging.table_name}.tag_id").having(having).where("#{Tagging.table_name}.taggable_id IN(#{select(scoped_select).to_sql})")    
        else
          tagging_scope = tagging_scope.group("#{ActsAsTaggableOn::Tagging.table_name}.tag_id HAVING #{having}")
        end
